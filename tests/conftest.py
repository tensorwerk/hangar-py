...
def written_repo(repo):
    ...
    co.datacells.init_datacell(name='_dcell', shape=(5, 7), dtype=np.float64)
...
def variable_shape_written_repo(repo):
    co = repo.checkout(write=True)
    co.datacells.init_datacell(name='_dcell', shape=(10, 10), dtype=np.float64, variable_shape=True)
    co.commit('this is a commit message')
    co.close()
    yield repo
...
def repo_1_br_no_conf(request, repo):
    ...
    for idx in range(10, 20):
        dummyData[:] = idx
        co2.datacells['dummy'][str(idx)] = dummyData
        co2.datacells['dummy'][idx] = dummyData
...
def repo_2_br_no_conf(repo_1_br_no_conf):
    ...
    for idx in range(20, 30):
        dummyData[:] = idx
        co1.datacells['dummy'][str(idx)] = dummyData
        co1.datacells['dummy'][idx] = dummyData
