...
def repo_20_filled_samples(request, aset_samples_initialized_repo, array5by7) -> Repository:
    ...
    second_aset = co.columns.create_ndarray_column('second_aset', prototype=array5by7, backend=request.param)
...
def repo_300_filled_samples(request, aset_samples_initialized_repo, array5by7) -> Repository:
    co = aset_samples_initialized_repo.checkout(write=True)
    aset = co.columns.create_ndarray_column('aset', prototype=array5by7, backend=request.param)
    with aset:
        for i in range(300):
            array5by7[:] = i
            aset[i] = array5by7
...
def repo_20_filled_samples_meta(repo) -> Repository:
    ...
    co1.columns.create_ndarray_column(name='dummy', prototype=dummyData)
    for idx in range(10):
        dummyData[:] = idx
