...
def test_nested_context_manager_does_not_close_all_open(repo, backend1, backend2):
    ...
    fooaset = co.define_ndarray_column('foo', prototype=np.arange(10), backend=backend1)
    baraset = co.define_ndarray_column('bar', prototype=np.arange(10), backend=backend2, contains_subsamples=True)
