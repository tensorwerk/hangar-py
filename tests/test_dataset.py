...
class TestDatacell(object):
    ...
    def test_read_only_mode(self, written_repo):
        ...
        assert co.datacells.remove_dset is None
        assert len(co.datacells['_dset']) == 0
        co.close()
    ...
    def test_get_datacell(self, written_repo, array5by7):
        ...
        dsetOld = co.datacells.get('_dset')
    ...
    def test_remove_datacell(self, dset_backend, written_repo):
        co = written_repo.checkout(write=True)
        co.datacells.remove_dset('_dset')
        with pytest.raises(KeyError):
            co.datacells.remove_dset('_dset')

        co.datacells.init_datacell(name='_dset', shape=(5, 7), dtype=np.float64, backend=dset_backend)
    ...
    def test_init_again(self, dset_backend, repo, randomsizedarray):
        co = repo.checkout(write=True)
        co.datacells.init_datacell('dset', prototype=randomsizedarray, backend=dset_backend)
        with pytest.raises(LookupError):
            co.datacells.init_datacell('dset', prototype=randomsizedarray, backend=dset_backend)
        co.close()

    @pytest.mark.parametrize("dset_backend", ['00', '10'])
    def test_datacell_with_more_dimension(self, dset_backend, repo):
        co = repo.checkout(write=True)
        shape = (0, 1, 2)
        with pytest.raises(ValueError):
            co.datacells.init_datacell('dset', shape=shape, dtype=np.int, backend=dset_backend)
        shape = [1] * 31
        dset = co.datacells.init_datacell('dset1', shape=shape, dtype=np.int, backend=dset_backend)
        assert len(dset._schema_max_shape) == 31
        shape = [1] * 32
        with pytest.raises(ValueError):
            # maximum tensor rank must be <= 31
            co.datacells.init_datacell('dset2', shape=shape, dtype=np.int, backend=dset_backend)
        co.close()

    @pytest.mark.parametrize("dset_backend", ['00', '10'])
    def test_datacell_with_empty_dimension(self, dset_backend, repo):
        co = repo.checkout(write=True)
        arr = np.array(1, dtype=np.int64)
        dset = co.datacells.init_datacell('dset1', shape=(), dtype=np.int64, backend=dset_backend)
        dset['1'] = arr
        co.commit('this is a commit message')
        dset = co.datacells.init_datacell('dset2', prototype=arr)
        dset['1'] = arr
        co.commit('this is a commit message')
        co.close()
        co = repo.checkout()
        dset1 = co.datacells['dset1']
        dset2 = co.datacells['dset2']
        assert np.allclose(dset1['1'], arr)
        assert np.allclose(dset2['1'], arr)
        co.close()

    @pytest.mark.parametrize("dset_backend", ['00', '10'])
    def test_datacell_with_int_specifier_as_dimension(self, dset_backend, repo):
        co = repo.checkout(write=True)
        arr = np.arange(10, dtype=np.int64)
        dset = co.datacells.init_datacell('dset1', shape=10, dtype=np.int64, backend=dset_backend)
        dset['1'] = arr
        co.commit('this is a commit message')
        arr2 = np.array(53, dtype=np.int64)
        dset = co.datacells.init_datacell('dset2', prototype=arr2)
        dset['1'] = arr2
        co.commit('this is a commit message')
        co.close()
        co = repo.checkout()
...
class TestDataWithFixedSizedDatacell(object):
    ...
    def test_iterating_over(self, dset1_backend, dset2_backend, dset3_backend, repo, randomsizedarray):
        co = repo.checkout(write=True)
        all_tensors = []
        dset1 = co.datacells.init_datacell('dset1', prototype=randomsizedarray, backend=dset1_backend)
        dset2 = co.datacells.init_datacell('dset2', shape=(2, 2), dtype=np.int, backend=dset2_backend)
        dset3 = co.datacells.init_datacell('dset3', shape=(3, 4), dtype=np.float32, backend=dset3_backend)
        ...
        for dname, dset in co.datacells.items():
        ...
        tensors_in_the_order = iter(all_tensors)
        for dset in co.datacells.values():
    ...
    def test_get_data(self, written_repo, array5by7):
        ...
        co.close()
        co = written_repo.checkout()
        assert np.allclose(co.datacells['_dset']['1'], co.datacells.get('_dset').get('1'), array5by7)
        co.close()
    ...
    def test_add_data_str_keys(self, written_repo, array5by7):
        ...
        co.close()
        co = written_repo.checkout()
        assert np.allclose(co.datacells['_dset']['1'], array5by7)
        assert np.allclose(co.datacells['_dset']['2'], array5by7)
        co.close()
    ...
    def test_add_data_int_keys(self, written_repo, array5by7):
        ...
        co = written_repo.checkout()
        assert np.allclose(co.datacells['_dset'][1], array5by7)
        assert np.allclose(co.datacells['_dset'][2], secondArray)
        co.close()
    ...
    def test_add_data_mixed_int_str_keys(self, written_repo, array5by7):
        ...
        co = written_repo.checkout()
        assert np.allclose(co.datacells['_dset'][1], array5by7)
        assert np.allclose(co.datacells['_dset']['1'], newFirstArray)
        assert np.allclose(co.datacells['_dset'][2], secondArray)
        assert np.allclose(co.datacells['_dset']['2'], thirdArray)
        co.close()

    def test_add_with_wrong_argument_order(self, w_checkout, array5by7):
        dset = w_checkout.datacells['_dset']
        with pytest.raises(ValueError):
            dset.add('1', array5by7)

    def test_multiple_data_single_commit(self, written_repo, array5by7):
        co = written_repo.checkout(write=True)
        co.datacells['_dset'].add(array5by7, '1')
        new_array = np.zeros_like(array5by7)
        co.datacells['_dset']['2'] = new_array
        co.commit('this is a commit message')
        co.close()
    ...
    def test_multiple_data_multiple_commit(self, written_repo, array5by7):
        ...
        co = written_repo.checkout(write=True)
        co.datacells['_dset']['3'] = new_new_array
        co.commit('this is a commit message')
        co.close()
    ...
    def test_remove_data(self, written_repo, array5by7):
        ...
        new_array = np.zeros_like(array5by7)
        co.datacells['_dset']['2'] = new_array
        co.datacells['_dset']['3'] = new_array + 5
        co.commit('this is a commit message')
        co.close()

        co = written_repo.checkout(write=True)
        co.datacells['_dset'].remove('1')
        del co.datacells['_dset']['3']
        co.commit('this is a commit message')
        co.close()

        co = written_repo.checkout()
        with pytest.raises(KeyError):
            co.datacells['_dset']['1']
        with pytest.raises(KeyError):
            co.datacells['_dset']['3']
        assert len(co.datacells['_dset']) == 1
        assert np.allclose(co.datacells['_dset']['2'], new_array)
        co.close()
    ...
    def test_remove_all_data(self, written_repo, array5by7):
        ...
        co = written_repo.checkout(write=True)
        co.datacells.init_datacell('_dset', prototype=array5by7)
        co.datacells['_dset']['1'] = array5by7
        co.commit('this is a commit message')
        co.close()
        co = written_repo.checkout()
    ...
    @pytest.mark.parametrize("dset1_backend", ['00', '10'])
    @pytest.mark.parametrize("dset2_backend", ['00', '10'])
    def test_multiple_datacells_single_commit(self, dset1_backend, dset2_backend, written_repo, randomsizedarray):
        co = written_repo.checkout(write=True)
        dset1 = co.datacells.init_datacell('dset1', prototype=randomsizedarray, backend=dset1_backend)
        dset2 = co.datacells.init_datacell('dset2', prototype=randomsizedarray, backend=dset2_backend)
        dset1['arr'] = randomsizedarray
        dset2['arr'] = randomsizedarray
        co.commit('this is a commit message')
        co.close()
        co = written_repo.checkout()
        assert np.allclose(co.datacells['dset1']['arr'], randomsizedarray)
        assert np.allclose(co.datacells['dset2']['arr'], randomsizedarray)
        co.close()
    ...
    def test_prototype_and_shape(self, dset1_backend, dset2_backend, repo, randomsizedarray):
        co = repo.checkout(write=True)
        dset1 = co.datacells.init_datacell(
            'dset1', prototype=randomsizedarray, backend=dset1_backend)
        dset2 = co.datacells.init_datacell(
        ...
        co = repo.checkout()
        assert np.allclose(co.datacells['dset1']['arr1'], newarray)
        assert np.allclose(co.datacells['dset2']['arr'], newarray)
        co.close()

    def test_samples_without_name(self, repo, randomsizedarray):
        co = repo.checkout(write=True)
        dset = co.datacells.init_datacell('dset', prototype=randomsizedarray)
        with pytest.raises(ValueError):
            dset.add(randomsizedarray)

        dset_no_name = co.datacells.init_datacell('dset_no_name',
    ...
    def test_different_data_types_and_shapes(self, repo):
        ...
        dset = co.datacells.init_datacell('dset', shape=shape, dtype=dtype)
        dset['1'] = arr
    ...
    @pytest.mark.parametrize("dset_backend", ['00', '10'])
    def test_writer_context_manager_datacell_add_sample(self, dset_backend, repo, randomsizedarray):
        co = repo.checkout(write=True)
        dset = co.datacells.init_datacell('dset', prototype=randomsizedarray, backend=dset_backend)
        with co.datacells['dset'] as dset:
            dset.add(randomsizedarray, '1')
        co.commit('this is a commit message')
        co.close()
        co = repo.checkout()
        assert np.allclose(co.datacells['dset']['1'], randomsizedarray)
        co.close()
    ...
    @pytest.mark.parametrize("dset_backend", ['00', '10'])
    def test_datacell_context_manager_dset_sample_and_metadata_add(self, dset_backend, repo, randomsizedarray):
        co = repo.checkout(write=True)
        dset = co.datacells.init_datacell('dset', prototype=randomsizedarray, backend=dset_backend)
        with co.datacells['dset'] as dset:
            dset.add(randomsizedarray, '1')
        ...
        co = repo.checkout()
        assert np.allclose(co.datacells['dset']['1'], randomsizedarray)
        assert np.allclose(co.datacells['dset'].get('2'), newarr)
        assert co.metadata['key'] == 'val'
    ...
    def test_bulk_add(self, dset1_backend, dset2_backend, repo, randomsizedarray):
        ...
        co = repo.checkout()
        data1 = next(co.datacells['dset_no_name1'].values())
        data2 = next(co.datacells['dset_no_name2'].values())
...
class TestVariableSizedDatacell(object):
    ...
    def test_writer_can_create_variable_size_datacell(self, written_repo, dtype, test_shapes, shape, backend):
        ...
        wco.datacells.init_datacell('vardset', shape=shape, dtype=dtype, variable_shape=True, backend=backend)
        d = wco.datacells['vardset']
    ...
    def test_writer_reader_can_create_read_multiple_variable_size_datacell(self, written_repo, dset_specs):
        repo = written_repo
        wco = repo.checkout(write=True)
        arrdict = {}
        for dset_spec in dset_specs:
            dset_name, test_shapes, max_shape, backend, dtype = dset_spec
            wco.datacells.init_datacell(dset_name, shape=max_shape, dtype=dtype, variable_shape=True, backend=backend)

            arrdict[dset_name] = {}
            for idx, shape in enumerate(test_shapes):
                arr = (np.random.random_sample(shape) * 10).astype(dtype)
                arrdict[dset_name][str(idx)] = arr
                wco.datacells[dset_name][str(idx)] = arr

        for dset_k in arrdict.keys():
            for samp_k, v in arrdict[dset_k].items():
                # make sure they are good before committed
                assert np.allclose(wco.datacells[dset_k][samp_k], v)
        ...
        for dset_k in arrdict.keys():
            for samp_k, v in arrdict[dset_k].items():
                # make sure they are good before committed
                assert np.allclose(wco.datacells[dset_k][samp_k], v)
                assert np.allclose(rco.datacells[dset_k][samp_k], v)
        wco.close()
        rco.close()
...
class TestMultiprocessDatacellReads(object):
    ...
    def test_external_multi_process_pool(self, repo, backend):
        ...
        masterCmtList = []
        co = repo.checkout(write=True)
        co.datacells.init_datacell(name='_dset', shape=(20, 20), dtype=np.float32, backend=backend)
        masterSampList = []
        for cIdx in range(2):
            if cIdx != 0:
                co = repo.checkout(write=True)
