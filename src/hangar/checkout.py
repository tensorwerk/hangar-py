...
class CheckoutIndexer(object):
    """Access arraysets directly from checkouts, allowing construction of dataset "views"
    """
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # Arrayset Parsing
            if (asetsIdx is Ellipsis) or (isinstance(asetsIdx, slice) and (
                    not any([asetsIdx.start, asetsIdx.stop, asetsIdx.step]))):
                asets = list(self._arraysets._arraysets.values())
                ...
                raise TypeError(f'Arraysets idx: {asetsIdx} type: {type(asetsIdx)}')
            nAsets = len(asets)
            asetnames = [aset.name for aset in asets]
            ArraysetData = namedtuple('ArraysetData', asetnames)

            # Sample Parsing
            if isinstance(sampleNames, (str, int)):
                sampleNames = [sampleNames]
            elif not isinstance(sampleNames, (tuple, list)):
                raise TypeError(f'Samples idx: {sampleNames} type: {type(sampleNames)}')
        ...
        try:
            ...
            if (nAsets > 1) and (nSamples > 1):
                raise SyntaxError(
                    ...
                    'arraysets in `set` operation in current implementation')
