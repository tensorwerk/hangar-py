...
class HDF5_00_Reader(object):

    def __init__(self, repo_path, schema, instance, is_staged=False):

        self.pth: str = repo_path
        self.is_staged: bool = is_staged
        ...
        self.fh: h5py.File = None

    def open(self):
        HDF_ATTRS = config.get('hangar.hdf5.attributes.keys')

        if self.is_staged is True:
            FDIR = config.get('hangar.repository.stage_data_dir')
        else:
            FDIR = config.get('hangar.repository.store_data_dir')

        fpth = pjoin(self.path, FDIR, f'hdf_{self.schema}', f'{self.instance}.hdf5')
        self.fh = h5py.File(fpth, mode='r', swmr=True, libver='latest')
        self.dtype = np.typeDict[self.fh['/'].attrs[HDF_ATTRS['schema_dtype']]]

    def close(self):
        self.fh.close()

    def read_data(self, hashVal):
        '''Read data from an hdf5 file handle at the specified locations
        ...
        srcSlc = (self.slcExpr[dsetIdx], *(self.slcExpr[0:x] for x in dataShape))
        destSlc = None
        destArr = np.empty((hashVal.shape), self.dtype)
        self.fh[dsetCol].read_direct(destArr, srcSlc, destSlc)
        return destArr


class HDF5_00_Writer(HDF5_00_Reader):
    ...
    def open(self):
        FDIR = config.get('hangar.repository.stage_data_dir')
        fpth = pjoin(self.path, FDIR, f'hdf_{self.schema}', f'{self.instance}.hdf5')
        self.fh = h5py.File(fpth, mode='a', libver='latest')
        self.fh.swmr_mode = True

        HDF_ATTRS = config.get('hangar.hdf5.attributes.keys')
        self.dtype = np.typeDict[self.fh['/'].attrs[HDF_ATTRS['schema_dtype']]]
        self.hNextPath = self.fh['/'].attrs[HDF_ATTRS['next_location']][0]
        self.hIdx = self.fh['/'].attrs[HDF_ATTRS['next_location']][1]
        self.hMaxSize = self.fh['/'].attrs[HDF_ATTRS['collection_max_size']]
        self.hColsRemain = int(self.fh['/'].attrs[HDF_ATTRS['collections_remaining']])
    ...
    def __exit__(self, *exc):
        HDF_ATTRS = config.get('hangar.hdf5.attributes.keys')
        self.fh['/'].attrs.modify(HDF_ATTRS['next_location'], (self.hNextPath, self.hIdx))
        self.fh['/'].attrs.modify(HDF_ATTRS['collections_remaining'], self.hColsRemain)
        self.fh.flush()
    ...
    def add_tensor_data(self, array):
        '''verifies correctness of array data and performs write operation.
        ...
        self.fh[f'/{self.hNextPath}'].write_direct(array, srcSlc, destSlc)
        return str(self.instance), str(self.hNextPath), str(self.hIdx)




class HDF5_00_Handles(object):

    def __init__(self, repo_pth, mode):
